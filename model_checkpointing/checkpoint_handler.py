...
def save_model_and_optimizer_sharded(model, rank, cfg,optim=None):
    ...
    with FSDP.state_dict_type(model, StateDictType.SHARDED_STATE_DICT):
        ...
        metadata = dist_cp.save_state_dict(
...
def save_model_checkpoint(
    ...
    if rank == 0:
        ...
        cfg.model_name = cfg.model_name.replace("/","+")
        # create save path
        folder_name = (
        cfg.dist_checkpoint_root_folder
        + "/"
        ...
        print("save_full_path:", save_full_path, flush=True)
        # save model
...
def load_model_checkpoint(model, rank, path):
    ...
    full_state_dict_model_path = (Path.cwd() / path)
    # is it present...
