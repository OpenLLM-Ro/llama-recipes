...
def _load_instructions():
    ...
    if LOADED_INSTRUCTIONS == None:
        full_instructions = []
        data_path = Path("ft_datasets/ro_orca/train-00000-of-00002-6d275f30fa8e143f-slim-orca-dedup-ro.json")
        instructions = json.load(open(data_path, encoding="utf-8"))
        full_instructions.extend(instructions)
        data_path = Path("ft_datasets/ro_orca/train-00001-of-00002-20da825e60baa022-slim-orca-dedup-ro.json")
        instructions = json.load(open(data_path, encoding="utf-8"))
        full_instructions.extend(instructions)
...
def get_preprocessed_roorca_dataset(dataset_config, tokenizer, split, compute_stats=False, nproc=None):
    ...
    print("RoOrca max words:", max_words)
    ...
    def get_text(sample):
        ...
        for k, v in sample.items():
            ...
            if v["from"] == "human":
                ...
                print("Unrecognized {0} actor".format(v["from"]))
            
            c.append({"role": user, "content": v["value"]})

        c = format_conv(c)
        return {"prompt": c, "text": c}
    ...
    dataset = dataset.map(get_text, num_proc=nproc, remove_columns=["0", "1", "2", "3"], desc="Extract texts")
...
if __name__ == "__main__":
    ...
    get_preprocessed_roorca_dataset(None, tokenizer, "full", compute_stats=True)
